...
export async function POST(request: Request) {
  try {
    ...
    if (!sessionId || !Number.isFinite(playbackSeconds) || playbackSeconds < 0 || playbackSeconds > 86400 || !['visible', 'hidden', 'prerender', 'unloaded'].includes(visibility) || !Number.isFinite(playbackRate) || playbackRate <= 0 || playbackRate > 4) {
    ...
    const { data, error } = await admin.rpc('record_task_heartbeat', {
      p_session_id: sessionId,
      p_client_time_seconds: playbackSeconds,
      p_visibility_state: visibility,
      p_playback_rate: playbackRate,
      p_seek_detected: seekDetected,
      p_mouse_activity: mouseActivity,
    })
    if (error) {
      const status = String(error.message || '').includes('EXPIRED') || String(error.message || '').includes('INACTIVE') ? 409 : 500
      return NextResponse.json({ error: error.message || 'Heartbeat rejected' }, { status })
    }

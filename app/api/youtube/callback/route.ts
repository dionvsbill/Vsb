...
export async function GET(request:Request){
 const url=new URL(request.url);const code=url.searchParams.get('code');const state=url.searchParams.get('state');const cookie=(await import('next/headers')).cookies();const saved=cookie.get('youtube_oauth_state')?.value; if(!code||!state||!saved)return NextResponse.redirect(new URL('/dashboard/profile?error=oauth_state',request.url));const [userId,savedState]=saved.split(':');if(!userId||savedState!==state)return NextResponse.redirect(new URL('/dashboard/profile?error=oauth_state',request.url))
 try{
  const tokenRes=await fetch('https://oauth2.googleapis.com/token',{method:'POST',headers:{'content-type':'application/x-www-form-urlencoded'},body:new URLSearchParams({code,client_id:process.env.GOOGLE_CLIENT_ID!,client_secret:process.env.GOOGLE_CLIENT_SECRET!,redirect_uri:`${process.env.NEXT_PUBLIC_APP_URL}/api/youtube/callback`,grant_type:'authorization_code'})});const tokens=await tokenRes.json();if(!tokenRes.ok)throw new Error('Google token exchange failed')
  ...
  const channel=data.items[0];const admin=createAdminClient();const access=await admin.rpc('store_youtube_secret',{p_value:tokens.access_token,p_name:`youtube_access_${userId}`});if(access.error)throw access.error;let refreshId=null;if(tokens.refresh_token){const refresh=await admin.rpc('store_youtube_secret',{p_value:tokens.refresh_token,p_name:`youtube_refresh_${userId}`});if(refresh.error)throw refresh.error;refreshId=refresh.data}

...
export async function POST() {
  try {
    ...
    const { data: settings } = await supabase.from('platform_settings').select('entry_fee_ghs').eq('id',true).single()
    const amountGhs = Number(settings?.entry_fee_ghs ?? 160)
    ...
    const { error: ledgerError } = await admin.from('transactions').insert({user_id:user.id,type:'entry_fee',amount:amountGhs,currency:'GHS',paystack_ref:reference,status:'pending',metadata:{access_code:payload.data.access_code}})
    if (ledgerError) return NextResponse.json({ error:'Unable to create payment ledger' }, { status:500 })
